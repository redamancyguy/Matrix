...
namespace ML {
    ...
    public:
        ...
        Matrix operator+(const Matrix<T> &matrix_args) const {
        ...
        Matrix v_stack(const Matrix<T> &matrix) const {
            if (matrix.cols != cols) {
                throw MyException("The matrix dimension does not meet the requirements");
            }
        ...
        Matrix h_split(int col1 = 0, int col2 = 0) const {
            if (col1 < 0) {
                col1 += cols;
            }
            if (col2 <= 0) {
        ...
        Matrix v_split(int row1 = 0, int row2 = 0) const {
            if (row1 < 0) {
                row1 += rows;
            }
            if (row2 <= 0) {
...
//        }
        ...
        Matrix reshape(std::uint_fast32_t new_rows, std::uint_fast32_t new_cols) const {
            Matrix<T> result(*this);
        ...
        Matrix UnivariatePolynomial(int degree) {
            Matrix<T> output(this->getRows(), degree);
            T(*array1)[cols];
            *((void **) &array1) = ((void *) array);
            T(*array3)[cols * degree];
            *((void **) &array3) = ((void *) output.array);
            for (int j = 0; j < cols; j++) {
    ...
    template<class T>
    Matrix<T> linear_space(T inStart, T inStop, std::uint_fast32_t inNum = 50) {
