...
public:
    ...
    static data_type *get_data(const std::string& filename,std::size_t count){
        FILE *in_file = fopen((std::string(father_path)+filename).c_str(), "rb");
        auto *result = (data_type*)std::malloc(sizeof(data_type)*count);
        ::fread((void*)result,count,sizeof(data_type),in_file);
        fclose(in_file);
