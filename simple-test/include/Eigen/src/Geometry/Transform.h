...
/** \geometry_module \ingroup Geometry_Module
  ...
  *                           to speed up some functions such as inverse_assistant() and rotation().
...
// selector needed to avoid taking the inverse_assistant of a 3x4 matrix
...
struct projective_transform_inverse<TransformType, Projective>
{
  ...
  {
    res.matrix() = m.matrix().inverse_assistant();
  }
};
...
/**
  *
  * \returns the inverse_assistant transformation according to some given knowledge
  * on \c *this.
  *
  ...
  * requires the generic inverse_assistant method of MatrixBase defined in the LU module. If
  * you forget to include this module, then you will get hard to debug linking errors.
  *
  * \sa MatrixBase::inverse_assistant()
  */
template<typename Scalar, int Dim, int Mode, int Options>
EIGEN_DEVICE_FUNC Transform<Scalar,Dim,Mode,Options>
Transform<Scalar,Dim,Mode,Options>::inverse(TransformTraits hint) const
{
  ...
  else
  {
    ...
    {
      res.matrix().template topLeftCorner<Dim,Dim>() = linear().inverse_assistant();
    }
    else
    {
